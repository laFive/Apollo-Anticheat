package me.lafive.apollo.check.impl.reach;
		...
		if (event instanceof FlyingEvent) {
			...
			if (e.getAction().equals(EntityUseAction.ATTACK)) {
				...
				for (PacketLocation pl : getData().getEntityTracker().getEntity(target).getRecentLocations()) {
					...
					AxisAlignedBB expandedBB = targetBB.expand(0.1f, 0.1f, 0.1f);
				...
				if (distance == Double.MAX_VALUE) {
					...
					if (distance >= 3.13) {
						...
						if ((threshold += 6) > 20) {
							...
							handleViolation(100, "LastDistance:" + distance + " Threshold:" + threshold);
