...
public class AxisAlignedBB {
    ...
    public double distanceXZ(final double x, final double z) {
        ...
        return MathHelper.sqrt_double(Math.min(Math.pow(x - minX, 2.0), Math.pow(x - maxX, 2.0)) + Math.min(Math.pow(z - minZ, 2.0), Math.pow(z - maxZ, 2.0)));
    }
    
    public boolean containsXZ(final double n, final double n2) {
        return minX <= n && maxX >= n && minZ <= n2 && maxZ >= n2;
    }
